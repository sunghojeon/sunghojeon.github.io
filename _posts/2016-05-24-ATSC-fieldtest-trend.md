...
![그림 1 / 지상파UHD 방송방식 결정 검증 실험방송 시스템 구성도. ATSC3.0 기반 (a) 송출부: HEVC 실시간 인코더, ROUTE/MMT 스트리밍 서버, 부가서비스 시그널링 서버, 그리고 IP 다중화기 (b) 송신부: KBS관악산UHD실험국에 설치된 ATSC3.0 엑사이터와 고출력 송신기, 그리고 전송 파라미터 제어기 및 모니터링 장비 (c) 수신부: 고정 수신 성능 검증을 위한 필드테스트 측정차](/images/ATSC_KOBA_Figure1.jpg)
