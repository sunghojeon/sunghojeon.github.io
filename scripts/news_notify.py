...
def _vet_chunk(items: list[dict], infos: list[dict],
    ...
    prompt = (
        ...
        "라디오 광고·편성·시상)은 제외 — 라디오라도 주파수 회수·매체 종료 "
        "같은 정책 사안이면 keep,\n"
        "  (마) 규제기관(Anatel, FCC, 방미통위, 과기정통부 등)의 방송·주파수·"
        "측위·차세대미디어·AI 규제 정책, 공공 자문(consulta/consultation), "
        "제도 개편 — 특히 방송 서비스 종료·주파수 회수/재배치(UHD·DMB·AM·"
        "700MHz)·재허가는 최우선 keep. 단, 개별 통신사 요금·개별 기업 인허가 "
        "같은 통신 실무 잡보는 제외,\n"
        "  (바) 방송 시장·산업 지표 — 방송광고 매출, 시청률·매체 이용률 통계, "
        "방송평가·재허가 심사, 방송사 경영·재정, 미디어 소유 규제 집행. 방송"
        "정책 변화의 근거가 되는 신호이므로 기술 키워드가 없어도 keep,\n"
        "  (사) 드론·무인이동체·로봇배송 생태계 (정밀측위의 수요처) — 지자체 "
        "드론배송/드론 실증도시/드론 특별자유화구역 사업(서산·사천·양주·수원·"
        "울주·제주 등 어느 지자체든 항상 keep), K-드론시스템·드론 원격식별"
        "(Remote ID) 규제(해외 사례 포함), 무인이동체 산업 행사·정책, "
        "배송로봇·이동로봇 제도(국토부 특별법 등), 도로교통공단 교통신호정보·"
        "교통인프라 협력, LaneAI·디지털도로 AI·TPEG 차로별 교통정보. 이 "
        "묶음은 방송·측위 키워드가 전혀 없어도 keep — '단어만 겹침'으로 "
        "제외하지 말 것.\n\n"
