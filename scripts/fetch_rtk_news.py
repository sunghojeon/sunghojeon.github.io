#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Collect Broadcast RTK / ATSC 3.0 news into the timeline data store.
...
US, Korea, Brazil, and Spanish-language LatAm, then rewrites the block
between NEWS-TIMELINE:START / NEWS-TIMELINE:END markers in
scripts/news_timeline.html. That file is a data store for the pipeline
(news_notify.py diffs it to find new articles for the Telegram digest) —
it is NOT rendered on the website; the public In-the-news page was
removed on purpose.
...
dir for news_notify.py to fold into the Telegram digest.

Usage (from the repository root):
    python scripts/fetch_rtk_news.py                  # fetch & update the store
...
DATA_FILE = SCRIPT_DIR / "news_timeline.html"
...
# Handoff to news_notify.py (same run, same machine): everything the digest
# should report about the collection itself, not just the articles.
...
# Filled in over the run, written to REPORT_FILE at the end of main().
RUN_REPORT = {
...
def _entityish(term: str) -> bool:
    """Single capitalized words are usually just sentence-starters in EN/PT/ES
    headlines (Quando, Centro, Veja) — accept a one-word term only when its
    shape is distinctively name-like (inner capital, digit, & or .)."""
    if " " in term:
        return True
    return bool(re.search(r"[A-Za-z][A-Z]|\d|&|\.", term))


def mine_candidates(items: list[dict], covered: str) -> list[tuple[str, str]]:
    """Heuristic fallback: recurring name-like phrases not already covered.

    Returns (term, region) — region is the majority region of the articles
    the term appeared in. Deliberately conservative: a junk query added here
    pollutes the store for runs; the LLM path is the primary proposer.
    """
    ...
    stop = {"The", "This", "New", "With", "For", "From", "Says", "Win", "TV",
            ...
            # PT/ES sentence-starters seen in the wild
            "Quando", "Como", "Veja", "Saiba", "Entenda", "Centro", "Após",
            "Governo", "Confira", "Nova", "Novo", "Cómo", "Qué"}
    for it in items:
        for m in pat.findall(it["title"]):
            if m.split()[0] in stop or len(m) < 5 or not _entityish(m):
                continue
    ...
    for term, regions in ranked:
        if len(regions) >= 3:     # must recur across 3+ articles
...
def _proposal_prompt(items: list[dict], store: dict,
    ...
    for it in items:
        region_tally[it["region"]] = region_tally.get(it["region"], 0) + 1
    tally_line = " · ".join(f"{r} {n}" for r, n in sorted(region_tally.items()))
    ...
    return (
        ...
        f"{tag_block}"
        f"Articles fetched this run (by region: {tally_line}):\n"
        + "\n".join(lines) + "\n\n"
        "Based on what these articles actually discuss, propose up to 5 NEW "
        "queries that would catch important related coverage the current "
        "queries would miss — e.g. newly emerged companies, products, "
        "regulators, funding programs, standards bodies, or policy "
        "proceedings central to this space. Judge each candidate as a BEAT "
        "to follow, not a one-off event: prefer entities and proceedings "
        "likely to keep producing coverage for months (a regulator docket, a "
        "patent campaign, a rollout program) over a single announcement.\n"
        "Pay special attention to (a) article tags/hashtags visible in the "
        "excerpts, (b) entities recurring across several articles, and (c) "
        "REGIONAL BALANCE — the per-region tally above shows where coverage "
        "is thin; US and KR beats (FCC/NextGen TV deployments, 과기정통부/"
        "방통위 정책, KBS/MBC/SBS 차세대방송, broadcast-positioning trials) "
        "are chronically under-collected relative to BR, so actively look "
        "for missing queries there. Prefer precise proper-noun queries over "
        "broad topics. Do not duplicate or trivially rephrase current "
        "queries. Korean queries must avoid quoted OR expressions.\n\n"
        'Respond with ONLY a JSON array: '
        '[{"q": "...", "region": "US|KR|BR|MX", "why": "one short sentence '
        'IN KOREAN"}] — return [] if nothing new is warranted.'
    )
...
def evolve(store: dict, items: list[dict], use_llm: bool, auto_yes: bool) -> None:
    ...
    for p in proposals:
        ...
        if ok:
            ...
            RUN_REPORT["queries_added"].append(
                {"q": p["q"], "region": p["region"], "why": p.get("why", "")})
...
def main() -> int:
    ...
    if MARK_START in text and MARK_END in text:
        ...
        text = pattern.sub(lambda _: blockText, text, count=1)
    else:
        text = blockText + "\n"
